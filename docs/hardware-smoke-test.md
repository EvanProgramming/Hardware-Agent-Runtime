...
4. Call `compile_firmware` with `examples/sketches/BlinkHeartbeat`, the project ID, and FQBN `arduino:avr:uno` (Uno) or `esp32:esp32:esp32` (ESP32 Dev Module). The Uno path is locally compile-verified; the ESP32 Core must first be installed.
