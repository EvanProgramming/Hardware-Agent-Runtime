...
4. 调用 `compile_firmware`：`sketchPath` 为 `examples/sketches/BlinkHeartbeat`；Uno 的 `fqbn` 为 `arduino:avr:uno`，ESP32 Dev Module 为 `esp32:esp32:esp32:UploadSpeed=115200`。两条编译路径均已在本机实际验证；ESP32 需要先安装 Core。
