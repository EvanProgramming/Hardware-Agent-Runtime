...
使用兼容 Arduino Uno 的开发板和支持数据传输的 USB 线。不要选择蓝牙或系统调试串口。
...
4. 调用 `compile_firmware`：`sketchPath` 为 `examples/sketches/BlinkHeartbeat`，Uno 的 `fqbn` 为 `arduino:avr:uno`。本机已实际编译通过，约使用 1912 字节 Flash、211 字节 RAM。
