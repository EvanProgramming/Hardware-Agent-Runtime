...
使用同一份 `examples/sketches/BlinkHeartbeat`，但将 `fqbn` 设为 `esp32:esp32:esp32`。接板后先调用 `inspect_environment`，选择发现的 USB 串口。若上传连接阶段卡住，请在开始连接时按住 **BOOT**，待连接后松开；是否需要取决于开发板与 USB 转串口芯片。
...
Arduino 适配器不会硬编码 AVR 行为：发现、编译和上传均使用所选 FQBN。真实 ESP32 的上传与串口恢复要等物理开发板接上后验证。
